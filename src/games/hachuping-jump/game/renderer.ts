...
function drawPillar(
  ...
  const radius = w / 2;
  ctx.save();
  ctx.shadowColor = `hsla(${hue}, 85%, 65%, 0.55)`;
  ctx.shadowBlur = 16;
  ctx.beginPath();
  ...
  grad.addColorStop(0, `hsla(${hue}, 70%, 45%, 1)`);
  grad.addColorStop(0.5, `hsla(${hue}, 90%, 75%, 1)`);
...
function drawObstacles(ctx: CanvasRenderingContext2D, t: LetterboxTransform, obstacles: Obstacle[]): void {
  for (const obstacle of obstacles) {
    ...
    drawPillar(ctx, screenX, topScreenY, screenW, gapTopScreenY - topScreenY, 200);

    const gapBottomScreenY = toScreen(t, 0, gapBottomY).y;
    const groundScreenY = toScreen(t, 0, GROUND_Y).y;
    drawPillar(ctx, screenX, gapBottomScreenY, screenW, groundScreenY - gapBottomScreenY, 200);
